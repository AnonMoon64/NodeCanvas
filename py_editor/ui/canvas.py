...
class LogicEditor(QGraphicsView):
    ...
    def add_connection_by_id(self, from_node_id, from_pin, to_node_id, to_pin):
        """Create a connection between nodes by their IDs - used by AI assistant"""
        from_node = None
        to_node = None
        
        for node in self.nodes:
            # NodeItem stores its id in `id`
        ...
        if not from_node or not to_node:
            print(f"Connection failed: could not find nodes {from_node_id} -> {to_node_id}")
            return None
        
        # Find the pins
        from_pin_item = from_node.output_pins.get(from_pin)
        to_pin_item = to_node.input_pins.get(to_pin)
        
        if not from_pin_item or not to_pin_item:
            print(f"Connection failed: could not find pins {from_pin} -> {to_pin}")
            return None
        
        # Create connection
        return self.create_connection(from_pin_item, to_pin_item)
